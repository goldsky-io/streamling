...
fn oracle() -> Value {
    serde_json::from_str(include_str!("deep_v2_math_oracle.json")).unwrap()
}
#[test]
fn deep_exact_division_against_python_fraction_oracle() {
    let data = oracle();
...
#[test]
fn deep_avg_batch_partition_merge_against_python_fraction_oracle() {
    let data = oracle();
...
#[test]
fn deep_native_narrowing_ties_and_precision_edges_python_fraction_oracle() {
    let data = oracle();
