...
#[test]
fn confluent_multi_writer_mixed_nested_binary_decimal_matrix() {
    let mut rows = 0;
    let mut configs = 0;
...
fn schema_drift_diagnostic_compares_native_and_wide() {
...
fn writer_schema_drift_is_detected_or_preserves_number() {
...
fn native_negative_scale_avro_diagnostic() {
    for scale in [-2, 0, 2] {
...
#[test]
fn json_mixed_decimal_binary_and_null_slices() {
    let d = arb_batch(
        100,
        4,
        &[
...
fn reader_evolution_decimal_defaults_are_exact() {
...
#[test]
fn reader_added_nullable_decimal_fields_restore_typed_nulls() {
...
fn nonnull_decimal_default_parser_support_diagnostic() {
...
#[test]
fn union_root_framing_late_registration_and_large_scales() {
    let mut records = 0;
    let mut configs = 0;
