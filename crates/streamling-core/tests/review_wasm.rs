...
#[tokio::test]
async fn review_wasm_zero_comparison_preserves_existing_string_contract() {
    let output = run(
...
#[tokio::test]
async fn review_wasm_string_100_roundtrips_to_avro() {
    let output = run("row => ({amount: '100'})", None).await;
    assert_avro_decimal_value(&output[0], "100");
}
#[tokio::test]
async fn review_wasm_decimal_identity_preserves_value() {
