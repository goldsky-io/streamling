...
///
/// Used to turn DataFusion's cumulative `elapsed_compute` into a per-batch
/// millisecond delta without losing sub-millisecond compute (see
/// `MetricsRecorder::record_execution_plan_metrics`). Side-effect-free so the
/// math is unit testable with hand-fed durations.
