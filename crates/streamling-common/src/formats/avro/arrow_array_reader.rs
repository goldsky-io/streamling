        ...
        let arrays: ArrowResult<Vec<ArrayRef>> = struct_fields
            ...
            .map(|field| {
                ...
                let arr = match field.data_type() {
                    ...
                    // Feature 002 (Retire U256/I256): the FSB(32)+U256/I256 read
                    // arms used to materialize decimal logical-type values as raw
                    // 32-byte big-endian bytes. After the source-routing flip
                    // (`formats/avro/schema.rs`), wide-precision decimals route
                    // to decimal_arb (LargeBinary) and the decimal_arb read arm
                    // above handles them. No source produces FSB(32)+U256/I256
                    // metadata anymore.
