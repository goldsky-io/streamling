...
pub fn to_avro(name: &str, fields: &Fields) -> Schema {
    // The caller-supplied `name` is often a Kafka topic (e.g.
    // `my-cluster.raw.events`) which can contain hyphens. Hyphens are illegal in
    // Avro names, so replace them before the name reaches the parser.
...
mod tests {
    ...
    /// was derived from a Kafka topic. Topics may contain hyphens or
    /// underscores; hyphens are illegal in Avro names so `to_avro` replaces them,
    /// while underscores are already valid and pass through unchanged. Both must
    /// serialize instead of panicking in `Schema::parse_str`.
    #[test]
    fn test_topic_name_with_hyphen_or_underscore_is_sanitized() {
        use apache_avro::types::Value::*;
        ...
        for topic in ["my-cluster.raw.events", "my_cluster.raw.events"] {
            let batch = RecordBatch::try_new(
                arrow_schema.clone(),
            ...
            // Would previously panic in Schema::parse_str for the hyphenated topic.
            let avro_schema = to_avro(topic, &arrow_schema.fields);
