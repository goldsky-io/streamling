...
impl TempInput {
    fn new(mixed: bool) -> Self {
        ...
        fs::write(
            path.join("a.parquet"),
            include_bytes!("../../streamling-e2e/tests/fixtures/deep_v3/arb_s0_id1.parquet"),
        )
        .unwrap();
        if mixed {
            fs::write(
                path.join("b.parquet"),
                include_bytes!("../../streamling-e2e/tests/fixtures/deep_v3/arb_s2_id2.parquet"),
            )
            .unwrap();
        }
        Self(path)
...
#[tokio::test]
async fn v3_explicit_parquet_metadata_and_storage_flags_preserve_numeric_value() {
    let b = scan(false, true, false).await.unwrap();
    assert_eq!(amounts(&b), ["1"]);
}
#[tokio::test]
async fn v3_explicit_parquet_metadata_flags_reject_conflicting_scales() {
...
async fn v3_session_flags_do_not_override_explicit_default_format_diagnostic() {
