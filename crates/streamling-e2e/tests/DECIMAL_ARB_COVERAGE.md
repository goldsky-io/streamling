...
| `CASE` / plain `COALESCE` | ✅ **F2 fixed** (`DecimalArbExprRewrite` re-stamps metadata) | ✅ `edge_decimal_sql` CASE tests | same-scale branches; mixed-scale left untouched |
| `coalesce_meta(...)` | ✅ preserves first-arg metadata | ⚠️ | still works; plain `COALESCE` now also preserved (F2) |
...
| `BETWEEN` / `IN` (integer literal bound) | ✅ **F1b fixed** (`DecimalArbExprRewrite` desugars Between/InList) | ✅ `edge_sql_runnable::between_int_literals` | runs before TypeCoercion via FunctionRewrite |
...
**Real code findings still open:** **F4** (sink retries non-retriable errors → hang; filed
as STRM-6322 for the Streaming team), **F5** (multiply overflow near the precision cap —
likely *correct* behavior; a UX decision, not a bug).

**Fixed this pass:** **F1** (decimal_arb + integer *binary-op* coercion —
`ExprPlanner` inserts `to_decimal_arb_from_int`); **F1b** (`BETWEEN`/`IN` literal bounds —
`DecimalArbExprRewrite` FunctionRewrite desugars Between/InList before TypeCoercion);
**F2** (CASE/COALESCE metadata loss — same rewrite re-stamps via `decimal_arb_with_meta`);
**F3** (Decimal128/256 → Postgres point misplaced — `unscaled_to_numeric_string`);
**F6** (nested decimal_arb → JSON hex — recursive rewrite in `json.rs`); **F7** (nested
decimal at Avro sink — `record_schema_json` preserves logicalType).
