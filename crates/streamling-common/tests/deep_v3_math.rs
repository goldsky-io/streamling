...
#[test]
fn v3_constructor_planning_rejects_precision_and_scale_that_wrap_u32() {
...
#[test]
fn v3_constructor_runtime_rejects_precision_and_scale_that_wrap_u32() {
    let mut accepted = Vec::new();
    for integer in [false, true] {
        let udf: Box<dyn ScalarUDFImpl> = if integer {
            Box::new(ToDecimalArbFromIntFunc::new())
...
#[tokio::test]
async fn v3_public_sql_constructor_rejects_wrapped_declaration() {
    let ctx = session();
    let mut accepted = Vec::new();
    for sql in [
...
#[tokio::test]
async fn v3_mixed_integer_native_decimal_expression_chains_are_exact() {
    let mut checked = 0;
...
#[tokio::test]
async fn v3_unsigned_integer_extremes_and_scalar_broadcast_are_exact() {
    let ctx = session();
...
#[test]
fn v3_native_casts_cover_extreme_negative_scales_and_signed_ties() {
...
#[tokio::test]
async fn v3_float_mixing_and_negative_native_scale_fail_explicitly() {
    let ctx = session();
...
#[test]
fn v3_decimal_text_parser_checks_value_not_lexical_digit_count() {
    let accepted = [
...
#[test]
fn v3_all_binary_ops_preserve_empty_null_and_scalar_batch_shapes() {
...
#[test]
fn v3_remainder_sign_and_extreme_scale_differences_follow_integer_oracle() {
    let mut checked = 0;
    for (sl, sr) in [
        (0, 0),
        (0, 18),
...
#[test]
fn v3_precision_cap_accepts_exact_products_and_rejects_loss_of_information() {
...
#[test]
fn v3_native_precision_limit_rounding_carry_is_checked_after_rounding() {
...
#[tokio::test]
async fn v3_numeric_scalar_functions_match_native_values_or_reject_explicitly() {
    let ctx = session();
